...
using Qnx.Unturned.Enums;
using Qnx.Unturned.Extensions;
using Qnx.Unturned.Items;
...
public class PlayerBuffItems : MonoBehaviour, IPlayerComponent
{
    ...
    private void onUseableChanged(PlayerEquipment equipment)
    {
        ...
        GunSet = ItemService.Instance!.GetGunSet(equipment.itemID, equipment.state);
        ...
        ItemService.Instance.GetAttachmentBuffs(Attachments, equipment.state);
        if (Attachments.Count == 0)
            return;
    ...
    private void updateClothing(EClothing type, ushort id)
    {
        ...
        cloth = ClothingBuffs[type] = id != 0 ? ItemService.Instance!.GetItem(id) : null;
    ...
    private IEnumerator clothingSetUpdate()
    {
        ...
        ItemService.Instance!.GetClothingSets(ClothingSets, _clothesArray);
